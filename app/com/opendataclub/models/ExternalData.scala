...
class ExternalDataSourceRepository(dbConfig: DatabaseConfig[JdbcProfile]) {
  ...
  def extract(id: Long): Future[DataImport] = {
    val edsf = db.run(externalDataSources.filter(_.id === -1L).take(1).result.head)
    for {
      eds <- edsf
 ...
 */
case class ExternalDataSource(sourceId: Long, name: String, description: String, url: String, downloadUrl: String, createdAt: DateTime, updatedAt: DateTime, id: Long) {
